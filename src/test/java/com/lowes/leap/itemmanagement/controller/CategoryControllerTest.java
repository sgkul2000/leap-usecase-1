...
class CategoryControllerTest {
  ...
  public void deleteCategory() throws Exception {
    ...
    ResponseEntity<HttpStatus> responseEntity = new ResponseEntity<>(
      HttpStatus.OK
    );
    ...
    mockmvc
      .perform(
        delete("/api/category/{id}", category.getId())
          .contentType(MediaType.APPLICATION_JSON)
          .content(new ObjectMapper().writeValueAsString(category))
      )
      .andExpect(status().isOk());
  ...
  public void updateCategory() throws Exception {
    ...
    ResponseEntity<Category> createResponseEntity = new ResponseEntity<>(
      category,
      HttpStatus.CREATED
    );
    when(categoryService.createCategory(any(Category.class)))
    ...
    ResponseEntity<Category> updateResponseEntity = new ResponseEntity<>(
      updatedCategory,
      HttpStatus.OK
    );
    when(
      categoryService.updateCategory(eq(category.getId()), any(Category.class))
    )
    ...
    mockmvc
      .perform(
        put("/api/category/{id}", category.getId())
          .contentType(MediaType.APPLICATION_JSON)
      ...
      .andExpect(
        jsonPath("$.description", is(updatedCategory.getDescription()))
      );
  }
}
